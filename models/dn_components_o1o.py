...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, num_superclasses, hidden_dim, label_enc, sup_label_enc):
    """
    ...
    if training:
        ...
        superclass_labels = torch.cat([t['superclasses'] for t in targets])
        known_superclass_labels = superclass_labels.repeat(scalar, 1).view(-1)
        known_superclass_labels_expand = known_superclass_labels.clone()
        ...
        if label_noise_scale > 0:
            ...
            p2 = torch.rand_like(known_superclass_labels_expand.float())
            chosen_indice2 = torch.nonzero(p2 < (label_noise_scale)).view(-1)  # usually half of bbox noise
            new_label2 = torch.randint_like(chosen_indice2, 0, num_superclasses)  # randomly put a new one here
            known_superclass_labels_expand.scatter_(0, chosen_indice2, new_label2)
        ...
        input_sup_embed = sup_label_enc(known_superclass_labels_expand.long().to('cuda'))
        indicator2 = torch.ones([input_sup_embed.shape[0], 1]).cuda()
        input_sup_embed = torch.cat([input_sup_embed, indicator2], dim=1)
        ...
        padding_sup = torch.zeros(pad_size, hidden_dim).cuda()
        
        input_query_label = torch.cat([padding_label, tgt], dim=0).repeat(batch_size, 1, 1)
        input_query_bbox = torch.cat([padding_bbox, refpoint_emb], dim=0).repeat(batch_size, 1, 1)
        input_query_sup = torch.cat([padding_sup, tgt], dim=0).repeat(batch_size, 1, 1)
        ...
        if len(known_bid):
            ...
            input_query_sup[(known_bid.long(), map_known_indice)] = input_sup_embed
        ...
        mask_dict = {
            ...
            'known_lbs_bboxes': (known_labels, known_bboxs, known_superclass_labels),
            'know_idx': know_idx,
            'pad_size': pad_size
        }
        ...
        input_query_sup = tgt.repeat(batch_size, 1, 1)
        attn_mask = None
        mask_dict = None

    return input_query_label, input_query_bbox, input_query_sup, attn_mask, mask_dict


def dn_post_process(outputs_class, outputs_coord, outputs_sup, mask_dict):
    """
    ...
    if mask_dict and mask_dict['pad_size'] > 0:
        ...
        output_known_sup = outputs_sup[:, :, :mask_dict['pad_size']]

        outputs_class = outputs_class[:, :, mask_dict['pad_size']:, :]
        outputs_coord = outputs_coord[:, :, mask_dict['pad_size']:, :]
        outputs_sup = outputs_sup[:, :, mask_dict['pad_size']:]

        mask_dict['output_known_lbs_bboxes']=(output_known_class,output_known_coord, output_known_sup)
    return outputs_class, outputs_coord, outputs_sup
...
def prepare_for_loss(mask_dict):
    ...
    output_known_class, output_known_coord, output_known_sup = mask_dict['output_known_lbs_bboxes']

    known_labels, known_bboxs, known_sup_labels = mask_dict['known_lbs_bboxes']
    ...
    if len(output_known_class) > 0:
        ...
        output_known_sup = output_known_sup.permute(1, 2, 0, 3)[(bid, map_known_indice)].permute(1, 0, 2)
    
    num_tgt = known_indice.numel()
    return known_labels, known_bboxs, known_sup_labels, output_known_class, output_known_coord, output_known_sup, num_tgt
...
def tgt_loss_sup_ce(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    if len(tgt_labels_) == 0:
        return {
            'tgt_loss_sup_ce': torch.as_tensor(0.).to('cuda'),
            'tgt_class_error_sup': torch.as_tensor(0.).to('cuda'),
        }
    ...
    losses = {'tgt_loss_sup_ce': loss_ce}

    losses['tgt_class_error_sup'] = 100 - accuracy(src_logits_, tgt_labels_)[0]
    return losses
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        known_labels, known_bboxs, known_sup_labels, \
        output_known_class, output_known_coord, output_known_sup, num_tgt = prepare_for_loss(mask_dict)
        ...
        losses.update(tgt_loss_sup_ce(output_known_sup[-1], known_sup_labels, num_tgt, focal_alpha))
    else:
        ...
        losses['tgt_loss_sup_ce'] = torch.as_tensor(0.).to('cuda')
        losses['tgt_class_error'] = torch.as_tensor(0.).to('cuda')
        losses['tgt_class_error_sup'] = torch.as_tensor(0.).to('cuda')
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and 'output_known_lbs_bboxes' in mask_dict:
                ...
                l_dict = tgt_loss_sup_ce(output_known_sup[-1], known_sup_labels, num_tgt, focal_alpha)
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
            else:
                l_dict = dict()
                ...
                l_dict['tgt_loss_sup_ce'] = torch.as_tensor(0.).to('cuda')
                l_dict['tgt_class_error_sup'] = torch.as_tensor(0.).to('cuda')
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
    return losses